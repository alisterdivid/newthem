...
class Quform_Admin_Page_Forms_List extends Quform_Admin_Page
{
    ...
    protected function enqueueScripts()
    {
        ...
        wp_localize_script('quform-forms', 'quformFormsListL10n', array(
            ...
            'saveTableSettingsNonce' => wp_create_nonce('quform_save_forms_table_settings'),
    ...
    {
        if ( ! Quform::currentUserCan('quform_list_forms')) {
    ...
    protected function addPageMessages()
    {
        ...
        if ($activated > 0) {
            $this->addMessage('success', sprintf(_n('Form activated', '%d forms activated', $activated, 'quform'), $activated));
        }

        $deactivated = (int) Quform::get($_GET, 'deactivated');
        if ($deactivated > 0) {
            $this->addMessage('success', sprintf(_n('Form deactivated', '%d forms deactivated', $deactivated, 'quform'), $deactivated));
        }

        $duplicated = (int) Quform::get($_GET, 'duplicated');
        if ($duplicated > 0) {
            $this->addMessage('success', sprintf(_n('Form duplicated', '%d forms duplicated', $duplicated, 'quform'), $duplicated));
        }

        $trashed = (int) Quform::get($_GET, 'trashed');
        if ($trashed > 0) {
            $this->addMessage('success', sprintf(_n('Form moved to the Trash', '%d forms moved to the Trash', $trashed, 'quform'), $trashed));
        }

        $untrashed = (int) Quform::get($_GET, 'untrashed');
        if ($untrashed > 0) {
            $this->addMessage('success', sprintf(_n('Form restored', '%d forms restored', $untrashed, 'quform'), $untrashed));
        }

        $deleted = (int) Quform::get($_GET, 'deleted');
        if ($deleted > 0) {
            $this->addMessage('success', sprintf(_n('Form deleted', '%d forms deleted', $deleted, 'quform'), $deleted));
        }
