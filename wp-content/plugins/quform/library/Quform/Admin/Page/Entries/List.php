...
class Quform_Admin_Page_Entries_List extends Quform_Admin_Page_Entries
{
    ...
    protected function enqueueScripts()
    {
        ...
        wp_localize_script('quform-entries-list', 'quformEntriesListL10n', array(
            ...
            'saveTableSettingsNonce' => wp_create_nonce('quform_save_entries_table_settings'),
    ...
    protected function processTrashAction($entryIds, $nonce)
    {
        ...
        if ( ! Quform::currentUserCan('quform_delete_entries')) {
            return array('error' => self::NO_PERMISSION);
        }
    ...
    protected function addPageMessages()
    {
        ...
        if ($read > 0) {
            $this->addMessage('success', sprintf(_n('Entry marked as read', '%d entries marked as read', $read, 'quform'), $read));
        }

        $unread = (int) Quform::get($_GET, 'unread');
        if ($unread > 0) {
            $this->addMessage('success', sprintf(_n('Entry marked as unread', '%d entries marked as unread', $unread, 'quform'), $unread));
        }

        $trashed = (int) Quform::get($_GET, 'trashed');
        if ($trashed > 0) {
            $this->addMessage('success', sprintf(_n('Entry moved to the Trash', '%d entries moved to the Trash', $trashed, 'quform'), $trashed));
        };

        $untrashed = (int) Quform::get($_GET, 'untrashed');
        if ($untrashed > 0) {
            $this->addMessage('success', sprintf(_n('Entry restored', '%d entries restored', $untrashed, 'quform'), $untrashed));
        };

        $deleted = (int) Quform::get($_GET, 'deleted');
        if ($deleted > 0) {
            $this->addMessage('success', sprintf(_n('Entry deleted', '%d entries deleted', $deleted, 'quform'), $deleted));
        };
    ...
    public function getNavHtml(array $form = null, array $extra = array())
    {
        $extra[40] = sprintf(
            ...
            sprintf(esc_html__('Entries for %s', 'quform'), $form['name'])
        );
