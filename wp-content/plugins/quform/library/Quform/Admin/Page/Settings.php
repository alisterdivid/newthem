...
class Quform_Admin_Page_Settings extends Quform_Admin_Page
{
    ...
    protected function enqueueScripts()
    {
        ...
        wp_localize_script('quform-settings', 'quformSettingsL10n', array(
            ...
            'errorVerifying' => sprintf(__('An error occurred verifying the license key, please try again. If this problem persists, see %sthis page%s.', 'quform'), '<a href="http://support.themecatcher.net/quform-wordpress/troubleshooting/common-problems/an-error-occurred-verifying-the-license-key" target="_blank">', '</a>'),
    ...
    protected function getServerCompatibility()
    {
        ...
        if (version_compare($server['phpVersion'], '5.0.0', '<')) {
            $compatibility['php']['error'] = __('The plugin requires PHP version 5 or later.', 'quform');
        }
        ...
        if (version_compare($server['wordpressVersion'], '3.8', '<')) {
            $compatibility['wordpress']['error'] = __('The plugin requires WordPress version 3.8 or later.', 'quform');
        }

        if ( ! $server['gdEnabled']) {
            $compatibility['gd']['error'] = __('The plugin requires the GD image library for the CAPTCHA element, please ask your host to install it.', 'quform');
        }

        if ( ! $server['ftEnabled']) {
            $compatibility['ft']['error'] = __('The plugin requires the FreeType library for the CAPTCHA element, please ask your host to install it.', 'quform');
        }

        if ( ! is_dir($server['tmpDir'])) {
            $compatibility['tmp']['error'] = __('The plugin temporary directory does not exist, please create the directory and make it writable.', 'quform');
        } else if ( ! wp_is_writable($server['tmpDir'])) {
            $compatibility['tmp']['error'] = __('The plugin temporary directory is not writable.', 'quform');
        }
    ...
    protected function getDisableableScripts()
    {
        return array(
            ...
            'smooth_scroll' => array(
                'disabled' => $this->options->get('disabledScripts.smoothScroll'),
                'name' => 'Smooth Scroll',
                'version' => 'v2.2.0',
