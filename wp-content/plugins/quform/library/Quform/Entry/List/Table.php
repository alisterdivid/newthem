...
class Quform_Entry_List_Table extends WP_List_Table
{
    ...
    protected function get_views()
    {
        ...
        $views['all'] = sprintf(
            ...
            $this->repository->getEntryCount($this->form->getId())
        );
        ...
        $views['unread'] = sprintf(
            ...
            $this->repository->getEntryCount($this->form->getId(), true)
        );
        ...
        $views['read'] = sprintf(
            ...
            $this->repository->getEntryCount($this->form->getId(), false)
        );
        ...
        $views['trash'] = sprintf(
            ...
            $this->repository->getEntryCount($this->form->getId(), null, 'trash')
        );
    ...
    protected function get_sortable_columns()
    {
        ...
        foreach ($this->getColumns() as $column) {
            $columns[$column] = array($column, $defaultOrderBy == $column);
        }
    ...
    protected function handle_row_actions($item, $column_name, $primary)
    {
        ...
        if ($item['status'] == 'normal') {
            ...
            if (Quform::currentUserCan('quform_edit_entries')) {
    ...
    protected function columnElement($item, $columnName)
    {
        ...
        if (isset($item[$columnName])) {
            $output = $this->form->setValueFromStorage($elementId, $item[$columnName])->getValueHtml($elementId);
        }
    ...
    {
        $currentUserId = get_current_user_id();
        $userOrderBy = get_user_meta($currentUserId, 'quform_entries_order', true);
