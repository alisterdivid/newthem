...
class Quform_Repository
{
    ...
    /**
     * Get all forms for display in the list table, including counts of read and unread entries
     *
     ...
     * @return  array           The new form config, also with new auto-generated ID
     */
    public function add(array $config)
    {
        ...
        $wpdb->insert($this->getFormsTableName(), array(
    ...
    /**
     * Get the export data for the form with the given ID
     *
     * @param  int    $id
     * @return string
     */
    public function getFormExportData($id)
    {
        $row = $this->find($id);

        if ($row != null) {
            return base64_encode($row['config']);
        }
...
WHERE `entries`.`form_id` = %d",$form->getId());
    ...
    public function uninstall()
    {
        ...
        $wpdb->query("DELETE FROM `{$wpdb->prefix}usermeta` WHERE `meta_key` = 'quform_recent_forms'");
        $wpdb->query("DELETE FROM `{$wpdb->prefix}usermeta` WHERE `meta_key` = 'quform_forms_order_by'");
        $wpdb->query("DELETE FROM `{$wpdb->prefix}usermeta` WHERE `meta_key` = 'quform_forms_order'");
        $wpdb->query("DELETE FROM `{$wpdb->prefix}usermeta` WHERE `meta_key` = 'quform_forms_per_page'");
        $wpdb->query("DELETE FROM `{$wpdb->prefix}usermeta` WHERE `meta_key` = 'quform_entries_order_by'");
        $wpdb->query("DELETE FROM `{$wpdb->prefix}usermeta` WHERE `meta_key` = 'quform_entries_order'");
        $wpdb->query("DELETE FROM `{$wpdb->prefix}usermeta` WHERE `meta_key` = 'quform_entries_per_page'");
        $wpdb->query("DELETE FROM `{$wpdb->prefix}usermeta` WHERE `meta_key` = 'quform_export_entries_format_settings'");
    }
}
