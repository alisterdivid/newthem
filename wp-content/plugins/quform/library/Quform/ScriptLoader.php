...
class Quform_ScriptLoader
{
    ...
    public function getScripts($isPreview = false)
    {
        ...
        if ( ! $this->options->get('disabledScripts.smoothScroll')) {
            $scripts['jquery-smooth-scroll'] = array(
                'url' => Quform::url('js/jquery.smooth-scroll.min.js'),
                'path' => QUFORM_PATH . '/js/jquery.smooth-scroll.min.js',
                'version' => '2.2.0'
            );
        }
        ...
        if (is_file($this->getCachePath($this->getCustomJsFilename()))) {
