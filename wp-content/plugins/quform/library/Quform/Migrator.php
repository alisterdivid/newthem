...
class Quform_Migrator
{
    ...
    public function getAllFormIds()
    {
        ...
        if ( ! Quform::currentUserCan('quform_full_access')) {
            wp_send_json(array(
                'type' => 'error',
    ...
    protected function convertForm(array $form)
    {
        ...
        $confirmation['resetForm'] = $form['reset_form_values'];
        ...
        foreach ($form['elements'] as $element) {
            ...
            switch ($element['type']) {
                ...
                case 'email':
                    ...
                    $eConfig['placeholder'] = $element['placeholder'];
        ...
        if ($form['send_autoreply'] &&
            ...
            $notification2['subject'] = $form['autoreply_subject'];
    ...
    public function migrateImportForm()
    {
        ...
        if ($result['type'] == 'success') {
            ...
            wp_send_json(array(
                'type' => 'success',
                'message' => sprintf(__('Form imported successfully, %sedit the form%s', 'quform'), '<a href="' . esc_url(admin_url('admin.php?page=quform.forms&sp=edit&id=' . $config['id'])) . '">', '</a>')
            ));
        } else {
            wp_send_json(array(
                'type' => 'error',
    ...
    protected function validateMigrateImportFormRequest()
    {
        ...
        if ( ! Quform::currentUserCan('quform_import_forms')) {
            wp_send_json(array(
                'type' => 'error',
                'message' => __('Insufficient permissions', 'quform')
            ));
    ...
    protected function validateMigrateSettingsRequest()
    {
        if (
            ...
            ! isset($_POST['migrate_license_key'], $_POST['migrate_license_key']) ||
